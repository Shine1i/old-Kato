<script lang="ts">
	...
	async function getEpisodes(isDub: boolean) {
		let messages = `No ${isDub ? 'dub' : 'sub'} currently available, trying to get ${
			!isDub ? 'dub' : 'sub'
		}`;
		let newEps = animeInfo.ids?.anilist
			? await Consument.getAnilistEpisodes(animeInfo.ids.anilist, userSettings.isDub)
			: animeInfo.ids?.mal
			? await Consument.getMALEpisodes(animeInfo.ids.mal, userSettings.isDub)
			: [];
		if (!newEps || newEps.length === 0) {
			toastStore.trigger({ message: messages });
			//try to get !dub if dub is not available
			newEps = animeInfo.ids?.anilist
				? await Consument.getAnilistEpisodes(animeInfo.ids.anilist, !userSettings.isDub)
				: animeInfo.ids?.mal
				? await Consument.getMALEpisodes(animeInfo.ids.mal, !userSettings.isDub)
				: [];
		...
		if (!newEps || newEps.length === 0) {
			toastStore.trigger({ message: 'No episodes currently available, please check later!' });
			return;
		}
		episodes = newEps;
	}
	...
	afterNavigate(async () => {
		...
		if (!browser) return;
		await getEpisodes(userSettings.isDub);
		infoChanged();

		settings.subscribe((val) => {
			if (userSettings.isDub == val.isDub) return;
			userSettings = val;
			reloadVideo();
		});
	});
	...
	async function reloadVideo() {
		...
		currentEpisode = await changeVidSrc(currentEpisode, episodes, userSettings.isDub, artRef);
	}
					...
					>
						...
						>
							<img
								...
								src={'//wsrv.nl/?url=' + `${episode.image}` + '&w=256&h=144&output=webp'}
								alt=""
								loading="lazy"
							/>
							<div
